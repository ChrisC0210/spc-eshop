...
<style>
  ...
  <div class="container text-h-1-4-1">
    ...
    <div class="card">
      @if(isset($new_item_banner))
      {{-- <img src="{{asset('assets/img/new.png')}}" class="card-img" alt="New Items"> --}}
      <img src="{{ route('shopping::showBannerImg', ['banner_id'=>$new_item_banner->banner_id]) }}" class="card-img" alt="New Items">

      <a href="{{ $new_item_banner->banner_url }}">
        <div class="mt-4 mb-4 card-img-overlay">
          <h5 class="spc-blue text-h-1-4-1">{{ $new_item_banner->banner_desc }}</h5>
          <h4 class="text-m-6-7-6 spc-blue">{{ $new_item_banner->banner_title }}</h4>
      ...
      <a href="">
        <div class="mt-4 mb-4 card-img-overlay">
          <h5 class="spc-blue text-h-1-4-1">new launch</h5>
    ...
    @foreach($new_item_products as $product)
      <a class="owl-carousel-card mb-4 mt-4 col col-lg-3 col-sm-6 d-flex" href="{!! route('shopping::productPage', ['product_cat_id'=>$product->product_cat_id, 'product_id'=>$product->product_id]) !!}">
        <div class="card flex-fill" style="border-radius: 4px;border: solid 1px #f5f5f5;">
          ...
          <img src="{{ $product->indexImage() }}" class="card-img-top" alt="p1">
